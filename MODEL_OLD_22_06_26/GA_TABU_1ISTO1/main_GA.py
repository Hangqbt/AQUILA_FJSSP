...
def run_ga_for_instance(inst_name, seed):
    ...
    # 2. Setup GA Hyperparameters (Aligned 1:1 with AO budget)
    Pop_size = 50
    Pc = 0.8
    Pm = 0.05
    Generations = 300
    ...
    for i in range(GA_Instance.Generations):
        ...
        # Update Generation Best & Stagnation Tracking
        gen_best = min(Fit)
        record_broken = False
        ...
        # --- TABU SEARCH INTEGRATION (1:1 with AO) ---
        # Trigger on Stagnation >= 3
        if stagnation_counter >= 3:
            best_idx = int(np.argmin(Fit))

            refined_chs, refined_fit = GA_Instance.tabu_local_search(
                CHS=C[best_idx],
                current_best_fit=Fit[best_idx],
                J=J,
        ...
        # Track metrics for the diagnostic graph
...
if __name__ == "__main__":
    ...
    # Save CSV files
    out_path_summary = Path("ga_tabu_benchmark_results_newinit_1to1.csv")
    out_path_summary.parent.mkdir(parents=True, exist_ok=True)
    df_results.to_csv(out_path_summary, index=False)

    out_path_seedwise = Path("ga_tabu_seedwise_fitness_results_newinit_1to1.csv")
    df_seedwise.to_csv(out_path_seedwise, index=False)

    out_path_seedwise_time = Path("ga_tabu_seedwise_time_results_newinit_1to1.csv")
