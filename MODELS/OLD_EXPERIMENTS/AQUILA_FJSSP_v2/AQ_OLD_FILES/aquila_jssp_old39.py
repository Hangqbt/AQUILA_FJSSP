...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features CORRECTED EXPLORATION CURVE, SMART MG (Dynamic Workload), Plateau Walking.
        Keeps 200-Iter Runway and bottom-50% Lévy Scatter.
        """
        self.J = J
    ...
    # =========================================================================
    # SMART HYBRID TABU SEARCH (DYNAMIC WORKLOAD)
    # =========================================================================
    def local_search(self, best_MS, best_OS, current_best_fit, num_steps, neighbors_per_step=10):
        ...
        for step in range(num_steps):
            ...
            if use_mg_heuristic:
                ...
                # Extract the dynamic workload (queue) of every machine from the simulator!
                machine_workloads = [decoder.Machines[m].End_time for m in range(self.M_num)]
            ...
            for _ in range(neighbors_per_step):
                ...
                if use_mg_heuristic and cp and len(cp) > 1:
                    ...
                    if random.random() < 0.5:
                        # SMART MACHINE REASSIGNMENT (Traffic Jam Fix)
                        flat_idx = 0
                        ...
                        if valid_machines:
                            # Calculate ECT (Earliest Completion Time) = Current Queue + Processing Time
                            ect_scores = {m: machine_workloads[m] + p_times[m] for m in valid_machines}

                            # Sort alternative machines by their dynamic queue score (lowest is best)
                            sorted_machines = sorted(valid_machines, key=lambda m: ect_scores[m])

                            # Pick randomly from the top 3 best machines to balance load while keeping variance
                            top_candidates = sorted_machines[:max(1, min(3, len(sorted_machines)))]
                            chosen_m = random.choice(top_candidates)

                            relative_idx = sum(1 for m in range(chosen_m) if p_times[m] != 9999)
    ...
    def optimize(self):
        ...
        while iter_num <= self.Maxiter:
            ...
            for i in range(self.N):
                ...
                if iter_num <= (0.666 * self.Maxiter):
                    if random.random() < 0.5:
                        ...
                        # =======================================================
                        # THE CORRECTED EXPLORATION CURVE
                        # Drops from 1.0 (Full Chaos) to ~0.13 (Heavy Convergence)
                        # =======================================================
                        if iter_num <= (0.333 * self.Maxiter):
                            Exploration_Rate = 1.0 - (iter_num / self.Maxiter)
                        else:
                            shifted_t = iter_num - (0.333 * self.Maxiter)
                            shifted_max = self.Maxiter * 0.666
                            Exploration_Rate = math.exp(-2.0 * (shifted_t / shifted_max))

                        for j in range(self.T0):
                            # FIXED INVERSION: Pick Elite (Diverse) when Exploration is High!
                            MS_new[j] = Elite_MS[j] if random.random() < Exploration_Rate else BEST_MS[j]
                        OS_new = self.pox_crossover(Elite_OS if random.random() < Exploration_Rate else BEST_OS,
                                                    X_OS[i])
            ...
            if stagnation_counter >= 15:
                ...
                # ==========================================================
                # BOTTOM-50% SCATTER (Keeping original constraints)
                # ==========================================================
                if not improved_in_sledgehammer:
                    num_to_scatter = self.N // 2
                    worst_indices = np.argsort(fitness)[-num_to_scatter:]
                    for w_idx in worst_indices:
                        massive_burst = max(10, self.T0 // 5)
            ...
            # ==========================================================
            # 200-ITERATION EARLY STOPPING (Keeping original constraints)
            # ==========================================================
            if Bestfitval < previous_best_fit:
                global_stagnation = 0
            else:
            ...
            if global_stagnation >= 200 and iter_num > (0.5 * self.Maxiter):
