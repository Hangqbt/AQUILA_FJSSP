...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features: 20% GS Init, 250-Iter Stopper, 80/20 Sledgehammer.
        UPGRADE: Swarm Personal Memory + OS Co-Mutation
        """
        self.J = J
            ...
            for _ in range(neighbors_per_step):
                ...
                if random.random() < 0.80:
                    ...
                    if candidates:
                        ...
                        # THE UPGRADE: Co-mutate the sequence to prevent gridlock!
            ...
            for i in range(self.N):
                ...
                if iter_num <= (0.666 * self.Maxiter):
                    if random.random() < 0.5:
                        ...
                        # THE UPGRADE: 3-Way Personal Genetic Memory
                        for j in range(self.T0):
                            r_val = random.random()
                            if r_val < Pc:
                                MS_new[j] = BEST_MS[j]
                            elif r_val < Pc + ((1.0 - Pc) / 2):
                                MS_new[j] = Elite_MS[j]
                            else:
                                MS_new[j] = X_MS[i][j]  # Retain Personal DNA!
