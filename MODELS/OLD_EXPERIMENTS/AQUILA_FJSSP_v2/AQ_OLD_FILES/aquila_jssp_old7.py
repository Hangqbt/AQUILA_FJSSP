...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        """
        Adaptive Hybrid Aquila Optimizer (AHAO) for FJSSP.
        Dynamically adjusts Island architecture based on puzzle complexity.
        Each Island Alpha performs localized discrete search.
        """
        self.J = J
        ...
        # The algorithm changes its own shape based on the factory size
        # =================================================================
        if self.T0 < 100:
            self.num_islands = 1  # Small puzzle: 1 massive swarm for heavy exploitation
        elif self.T0 < 200:
            self.num_islands = 2  # Medium puzzle: 2 sub-populations
        else:
            self.num_islands = 3  # Massive puzzle: 3 islands for maximum exploration

        self.island_size = self.N // self.num_islands
        self.migration_interval = 25  # How often the islands share their Alpha eagles

        # Fairly distribute the discrete LS budget so computational time remains stable
        self.ls_swaps_per_island = 200 // self.num_islands
        # =================================================================
    ...
    def local_search(self, best_cont_vector, current_best_fit, num_swaps):
        """
        Discrete Local Search applied dynamically to Island Alphas.
        Budget is determined by the Adaptive Configuration.
        """
    ...
    def optimize(self):
        ...
        for i in range(self.N):
            ...
            # Failsafe in case N doesn't divide perfectly by num_islands
            island_idx = min(island_idx, self.num_islands - 1)
        ...
        while iter_num <= self.Maxiter:
            ...
            # --- THE HYBRIDIZATION: Local Search on EVERY Island Alpha ---
            for k in range(self.num_islands):
                # Apply the allocated budget of discrete swaps to the Island's best eagle
                Island_BEST[k], Island_Bestfitval[k] = self.local_search(Island_BEST[k], Island_Bestfitval[k],
                                                                         self.ls_swaps_per_island)

                # Check if this Island's new buffed Alpha beat the Global Alpha
                if Island_Bestfitval[k] < Global_Bestfitval:
                    Global_Bestfitval = Island_Bestfitval[k]
                    Global_BEST = np.copy(Island_BEST[k])

                # Inject the improved Alpha back into the weakest eagle of its specific island
                start_idx = k * self.island_size
                end_idx = (k + 1) * self.island_size if k < self.num_islands - 1 else self.N
            ...
            # Only migrate if there is more than 1 island!
