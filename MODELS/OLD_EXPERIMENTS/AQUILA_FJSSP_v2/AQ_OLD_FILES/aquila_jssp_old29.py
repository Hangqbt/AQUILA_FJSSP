...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features Dual Decay (-2.0), Top 10% Multi-Drill, and MG Critical Path Local Search.
        """
        self.J = J
    ...
    def local_search(self, best_MS, best_OS, current_best_fit, num_steps, neighbors_per_step=10):
        ...
        for step in range(num_steps):
            # 1. Ask the X-Ray Machine (Decode) for the Critical Path
            decoder = Decode(self.J, self.Processing_time, self.M_num)
            ...
            for _ in range(neighbors_per_step):
                ...
                # If we successfully extracted a Critical Path
                if cp and len(cp) > 1:
                    # Pick a random operation that is physically causing the bottleneck
                    c_job, c_op, c_mach = random.choice(cp)

                    if random.random() < 0.5:
                        # ---------------------------------------------------
                        # MG MOVE 1: CRITICAL MACHINE REASSIGNMENT
                        # ---------------------------------------------------
                        # Calculate the exact flat index in the 1D MS array
                        flat_idx = 0
                        for k in range(1, c_job + 1):  # Keys in self.J are 1-based
                            flat_idx += self.J[k]
                        flat_idx += c_op

                        max_m = self.Feasible_Machines_Count[flat_idx] - 1
                        if max_m > 0:
                            new_m = random.randint(0, max_m)
                            temp_MS[flat_idx] = new_m
                            move_signature = f"CP_MS_{flat_idx}_{new_m}"
                    else:
                        # ---------------------------------------------------
                        # MG MOVE 2: CRITICAL SEQUENCE SWAP (BLOCK LOGIC)
                        # ---------------------------------------------------
                        # Find another operation on the CP that shares the SAME bottleneck machine
                        ...
                        if same_mach_ops:
                            ...
                            # Helper to find exact sequence index in OS
                ...
                # Failsafe: If CP logic couldn't generate a move, do a random swap
    ...
    def optimize(self):
        ...
        while iter_num <= self.Maxiter:
            ...
            if Bestfitval >= previous_best_fit:
                ...
                # ==========================================================
            # --- THE "TOP 10% MULTI-DRILL" TABU ENGINE ---
            # ==========================================================
