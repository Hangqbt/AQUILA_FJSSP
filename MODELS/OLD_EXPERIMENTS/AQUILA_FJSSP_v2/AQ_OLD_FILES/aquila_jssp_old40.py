...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features CORRECTED EXPLORATION, GREEDY MG, and FULL SWARM EVACUATION.
        Keeps the 200-Iteration Early Stopper.
        """
        self.J = J
            ...
            for _ in range(neighbors_per_step):
                ...
                if use_mg_heuristic and cp and len(cp) > 1:
                    ...
                    if random.random() < 0.5:
                        # REVERTED TO GREEDY MACHINE REASSIGNMENT
                        flat_idx = 0
    ...
    def optimize(self):
        ...
        while iter_num <= self.Maxiter:
            ...
            if stagnation_counter >= 15:
                ...
                # ==========================================================
                # FULL SWARM EVACUATION (96% Scatter)
                # If Sledgehammer fails, nuke 29 out of 30 eagles to break the gravity trap!
                # ==========================================================
                if not improved_in_sledgehammer:
                    # Identify the current best eagle index to keep it safe
                    best_eagle_idx = np.argmin(fitness)

                    for w_idx in range(self.N):
                        if w_idx != best_eagle_idx:
                            # Massive 33% mutation burst
                            massive_burst = max(15, self.T0 // 3)
            ...
            # ==========================================================
            # 200-ITERATION EARLY STOPPING (Active)
            # ==========================================================
            if Bestfitval < previous_best_fit:
                global_stagnation = 0
            else:
