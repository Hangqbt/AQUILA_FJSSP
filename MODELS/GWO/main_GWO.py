...
# Import the new GWO algorithm
from gwo_fjsp import Discrete_GWO_FJSSP
...
def Gantt(Machines, inst_name, save_path):
    ...
    plt.title(f'Scheduling Gantt Chart (GWO) - {inst_name}')
    plt.ylabel('Machines')
    plt.xlabel('Time')
    plt.tight_layout()
...
def plot_convergence(run_histories, inst_name, save_path):
    ...
    plt.fill_between(iterations, avg_history - std_history, avg_history + std_history,
                     color='green', alpha=0.2, label='Standard Deviation (Spread)')
    plt.plot(iterations, avg_history, label='Average Convergence', color='green', linewidth=2)
    plt.plot(iterations, best_history, label='Lowest (Best) Solution', color='red', linestyle='--', linewidth=2)
    plt.title(f'Convergence Analysis (GWO) - {inst_name}')
...
def plot_boxplot(run_best_objs, inst_name, save_path):
    ...
    plt.boxplot(run_best_objs, tick_labels=['Discrete GWO'], patch_artist=True,
...
def run_single_gwo(args):
    ...
    gwo = Discrete_GWO_FJSSP(J=J, Processing_time=Processing_time, M_num=M_num, Pop_size=POP_SIZE, Maxiter=MAX_ITER)
    best_chromosome, best_makespan, conv_history = gwo.optimize()
...
if __name__ == '__main__':
    ...
    POP_SIZE = 300
    MAX_ITER = 50
    DEFAULT_DATA_DIR = "./Data"  # Ensure this points to your instances
    ...
    for inst_name in instances:
        ...
        with ProcessPoolExecutor() as executor:
            outputs = list(executor.map(run_single_gwo, run_args))
    ...
    print("\n================ FINAL SUMMARY TABLE (GWO) ================\n")
    print(df_results)

    # Save CSV files
    out_path_summary = Path("gwo_benchmark_results.csv")
    ...
    df_seedwise_fitness.to_csv("gwo_seedwise_fitness_results.csv", index=False)
    df_seedwise_time.to_csv("gwo_seedwise_time_results.csv", index=False)
