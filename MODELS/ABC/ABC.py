...
class ABC_FJSSP:
    ...
    def critical_path_local_search(self, CHS, num_steps, Best_fit):
        best_CHS = np.copy(CHS)
        best_fit = Best_fit
        for _ in range(num_steps):
            neighbor = np.copy(best_CHS)
            if random.random() < 0.5:
    ...
    def optimize(self):
        ...
        for iter_num in range(self.Maxiter):
            ...
            for i in range(self.n_eb):
                ...
                if N_i > 0:
                    refined_chs, refined_fit = self.critical_path_local_search(Pop[i], N_i, Fitness[i])
