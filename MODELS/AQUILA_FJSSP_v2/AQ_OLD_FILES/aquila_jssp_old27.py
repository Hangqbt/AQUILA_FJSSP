...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features Cosine Annealing Decay and a Top 10% Multi-Drill Local Search Engine.
        """
        self.J = J
    ...
    def optimize(self):
        ...
        while iter_num <= self.Maxiter:
            ...
            for i in range(self.N):
                ...
                if iter_num <= (0.666 * self.Maxiter):
                    if random.random() < 0.5:
                        ...
                        # =======================================================
                        # COSINE ANNEALING PROBABILITY MECHANISM
                        # =======================================================
                        Pc = 0.5 * (1 + math.cos(math.pi * (iter_num / self.Maxiter)))
                        # =======================================================
            ...
            # ==========================================================
            num_elites = max(1, self.N // 10)  # Identifies the Top 10% (3 eagles)

            if stagnation_counter >= 15:
                # SLEDGEHAMMER: Drops 3 simultaneous deep-drills into different craters
                ...
                # PHASE 3 & 4 MAINTENANCE: Polishes the top 3 schedules independently
                sorted_idx = np.argsort(fitness)
                for rank in range(num_elites):
                    idx = sorted_idx[rank]
                    refined_MS, refined_OS, refined_fit = self.local_search(X_MS[idx], X_OS[idx], fitness[idx],
