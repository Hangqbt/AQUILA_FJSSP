...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features CORRECTED EXPLORATION, PURE RANDOM TABU (old28), and POX SWARM EVACUATION.
    ...
    # =========================================================================
    # PURE RANDOM TABU SEARCH (Reverted to old28 - Blind & High Variance)
    # =========================================================================
    def local_search(self, best_MS, best_OS, current_best_fit, num_steps, neighbors_per_step=10):
        ...
        for step in range(num_steps):
            ...
            for _ in range(neighbors_per_step):
                ...
                # 100% Blind Random Mutations - allows for non-critical "Prep Moves"!
                ...
                if not improved_in_sledgehammer:
                    ...
                    for w_idx in range(self.N):
                        if w_idx != best_eagle_idx:
                            # 1. Cross the trapped eagle with the Alpha to build a structural bridge
                            X_OS[w_idx] = self.pox_crossover(BEST_OS, X_OS[w_idx])
                            # 2. Mutate heavily to ensure it lands in a NEW crater near the bridge
