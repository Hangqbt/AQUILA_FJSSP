...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        ...
        Features: 20% GS Init, Strict Descent (<), 250-Iter Stopper.
        UPGRADE: 80/20 Hybrid Memetic Sledgehammer (Load Balancing + Random)
        """
        self.J = J
    ...
    # =========================================================================
    # 80/20 HYBRID SLEDGEHAMMER (Smart Load Balance + Blind Random)
    # =========================================================================
    def local_search(self, best_MS, best_OS, current_best_fit, num_steps, neighbors_per_step=10):
        ...
        for step in range(num_steps):
            ...
            for _ in range(neighbors_per_step):
                ...
                # 80% SMART: Move a job from heaviest machine to lightest machine
                if random.random() < 0.80:
                    workloads = np.zeros(self.M_num)
                    valid_map = []
                    flat_idx = 0
                    ...
                    for job_id, num_ops in self.J.items():
                        ...
                        for op_idx in range(num_ops):
                            p_times = self.Processing_time[job_idx][op_idx]
                            valid_m = [m for m, t in enumerate(p_times) if t != 9999]
                            chosen_m = valid_m[temp_MS[flat_idx]]
                            workloads[chosen_m] += p_times[chosen_m]
                            valid_map.append((flat_idx, chosen_m, valid_m, p_times))
                            flat_idx += 1

                    max_m = np.argmax(workloads)
                    min_m = np.argmin(workloads)

                    # Find operations on max_m that can be safely moved to min_m
                    candidates = [item for item in valid_map if item[1] == max_m and item[3][min_m] != 9999]

                    if candidates:
                        chosen = random.choice(candidates)
                        f_idx, c_m, v_m, p_times = chosen
                        new_rel_idx = v_m.index(min_m)
                        temp_MS[f_idx] = new_rel_idx
                        move_signature = f"SMART_{f_idx}_{new_rel_idx}"
                    else:
                        # Fallback to random OS swap if no valid machine move exists
                        idx1, idx2 = random.sample(range(self.T0), 2)
                        temp_OS[idx1], temp_OS[idx2] = temp_OS[idx2], temp_OS[idx1]
                        move_signature = f"OS_{min(idx1, idx2)}_{max(idx1, idx2)}"

                # 20% BLIND: Pure random chaos to make prep moves
                else:
                    if random.random() < 0.5:
                        ...
                        max_m_count = self.Feasible_Machines_Count[idx_m] - 1
                        if max_m_count > 0:
                            new_m = random.randint(0, max_m_count)
                            temp_MS[idx_m] = new_m
    ...
    def optimize(self):
        ...
        while iter_num <= self.Maxiter:
            ...
            if global_stagnation >= 250 and iter_num > (0.5 * self.Maxiter):
