...
class Aquila_Optimizer_FJSSP:
    def __init__(self, J, Processing_time, M_num, Pop_size, Maxiter):
        """
        The "Deep Drill" Hybrid Aquila Optimizer (HAO) for FJSSP.
        Architecture: Single Swarm + Stagnation-Triggered Deep Drill Tabu Search.
        """
        self.J = J
    ...
    def local_search(self, best_cont_vector, current_best_fit, num_steps):
        """
        --- Tabu Search (TS) ---
        Memory-guided neighborhood exploration.
        Budget dynamically scales based on stagnation (Deep Drill).
        """
        ...
        for _ in range(num_steps):  # Dynamic steps dictated by Stagnation Trigger
            neighborhood = []
            ...
            for neighbor_cont, neighbor_fit, signature in neighborhood:
                # 1. Aspiration Criterion: It breaks the global record!
                ...
                # 2. Standard Acceptance: Best available neighbor and NOT forbidden
    ...
    def optimize(self):
        ...
        # --- NEW: STAGNATION TRACKING FOR DEEP DRILL ---
        ...
        while iter_num <= self.Maxiter:
            ...
            for i in range(self.N):
                ...
                # Evaluate the micro-adjusted flight
            ...
            # =========================================================
            # --- STAGNATION TRIGGER: THE DEEP DRILL ---
            # =========================================================
            ...
            # Default Tabu steps (20 steps * 10 neighbors = 200 evaluations)
            tabu_steps = 20

            # If trapped for 15 iterations, trigger Deep Drill!
            if stagnation_counter >= 15:
                tabu_steps = 50  # 50 steps * 10 neighbors = 500 evaluations
                stagnation_counter = 0  # Reset frustration after Deep Drill
            # =========================================================

            # Hybridization: Tabu Search Only (with dynamic budget)
            BEST, Bestfitval = self.local_search(BEST, Bestfitval, tabu_steps)
