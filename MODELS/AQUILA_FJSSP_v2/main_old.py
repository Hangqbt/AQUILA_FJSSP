...
from aquila_jssp_old51 import Aquila_Optimizer_FJSSP
...
def plot_convergence(run_histories, inst_name, save_path):
    """Plots the average, best, and standard deviation convergence of the algorithm."""
    # Convert to numpy array for easy column-wise mathematical operations
    histories_matrix = np.array(run_histories)
    ...
    # 1. Plot the Standard Deviation as a shaded region to show exploration spread
    plt.fill_between(iterations,
                     avg_history - std_history,
                     avg_history + std_history,
                     color='blue', alpha=0.2, label='Standard Deviation (Spread)')

    # 2. Plot the Average Convergence line
    plt.plot(iterations, avg_history, label='Average Convergence', color='blue', linewidth=2)

    # 3. Plot the Lowest (Best) Solution found across all runs
...
if __name__ == '__main__':
    ...
    # Generate a global timestamp for this entire execution batch
    ...
    # Create run-specific subfolders to group the datasets automatically
    ...
    for inst_name in instance_names:
        try:
            ...
            # 1. Generate the Enhanced Convergence Plot (Pass the full history matrix)
